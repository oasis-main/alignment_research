...
4096-dimensional bipolar ($\pm 1$) hypervectors. Relation application
uses non-commutative binding: each relation $r$ is assigned a fixed
random circular shift $\sigma_r$, so that $\sigma_r(h) \neq
\sigma_r^{-1}(h)$ and a directed edge $h \xrightarrow{r} t$ is
distinguishable from its reverse. This is the standard construction for
encoding directed relational structure in hyperdimensional computing.
